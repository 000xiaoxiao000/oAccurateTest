  ...
  <section>
    ...
    <template v-else-if="payload?.report">
      <div class="hero-grid">
        ...
        <article class="hero-card accent-tia">
          <span>快照 TIA</span>
          <strong>{{ snapshotImpactCount }}</strong>
          <small>{{ snapshotImpactHint }}</small>
        </article>
      ...
      <section class="impact-grid">
        ...
        <div id="report-snapshot-tia" class="panel">
          <div class="panel-head"><h2>多语言快照 TIA</h2><span>{{ snapshotImpactCount }}</span></div>
          <div v-if="payload.testImpact?.reasons?.length" class="info-message">
            {{ payload.testImpact.reasons.join('；') }}
          </div>
          <div class="impact-list endpoint-list">
            <article v-for="item in paginatedSnapshotImpacts" :key="snapshotImpactKey(item)" class="impact-card endpoint-card">
            ...
            <div v-if="!snapshotImpactCount" class="empty-card">当前版本报告未得到可归因的快照 TIA 结果。</div>
          </div>
          <AppPagination
            v-if="snapshotImpactCount > 0"
            v-model:page="snapshotImpactPage"
            v-model:page-size="snapshotImpactPageSize"
            :total="snapshotImpactCount"
            item-name="来源"
...
import type { TestImpactCase } from '@/entities/coverage/model'
...
const snapshotImpactPage = ref(1)
const snapshotImpactPageSize = ref(4)
...
const snapshotImpactCount = computed(() => payload.value?.testImpact?.impactedCases?.length || 0)
const snapshotImpactHint = computed(() => {
  const impact = payload.value?.testImpact
  if (!impact) return '等待覆盖率报告关联'
  if (impact.impactedCaseCount) return `${impact.impactedCaseCount} 个用例来源`
  if (impact.impactedTraceCount) return `${impact.impactedTraceCount} 条 trace 来源`
  return impact.reasons?.[0] || '暂无快照命中'
})
...
const paginatedSnapshotImpacts = computed(() => {
  const impacts = payload.value?.testImpact?.impactedCases || []
  const start = (snapshotImpactPage.value - 1) * snapshotImpactPageSize.value
  return impacts.slice(start, start + snapshotImpactPageSize.value)
})
...
watch(snapshotImpactPageSize, () => {
  snapshotImpactPage.value = 1
})
...
watch(snapshotImpactCount, (total) => {
  const totalPages = Math.max(1, Math.ceil(total / snapshotImpactPageSize.value))
  if (snapshotImpactPage.value > totalPages) snapshotImpactPage.value = totalPages
})
...
function snapshotImpactKey(item: TestImpactCase) {
  return [item.caseName, item.traceId, item.buildId, item.testStage].filter(Boolean).join('|') || JSON.stringify(item)
}

...
const graphRoute = computed(() => `/p/${projectId.value}/my-snapshots/${snapshotId.value}/graph`)
...
const codeGraphRoute = computed(() => graph.value?.hasCodeLayer && graph.value.traceId ? `/p/${projectId.value}/map/code?traceId=${encodeURIComponent(graph.value.traceId)}&backRoute=${encodeURIComponent(graphRoute.value)}&backLabel=${encodeURIComponent('返回我的快照链路图')}` : undefined)

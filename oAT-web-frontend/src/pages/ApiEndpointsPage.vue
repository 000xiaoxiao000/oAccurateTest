  ...
  <section>
    <div class="page-header">
      <div>
        ...
        <h1>接口扫描与覆盖</h1>
        <p class="subtext">扫描源码包或制品包，按接口维度查看覆盖状态、命中次数和关联用例。</p>
      </div>
    ...
    <section class="panel list-panel">
      <div class="toolbar">
        ...
        <select v-model="coverage" class="text-input compact">
          <option value="">全部覆盖状态</option>
      ...
      <div class="toolbar sub-toolbar">
        ...
        <button class="ghost-button" type="button" @click="toggleUncoveredOnly">只看未覆盖</button>
      ...
      <template v-else>
        <article v-for="item in pagedItems" :key="item.key" class="endpoint-card" :class="item.covered ? 'covered' : 'uncovered'">
...
const coveredCount = computed(() => endpoints.value.filter((item) => item.covered).length)
const uncoveredCount = computed(() => endpoints.value.length - coveredCount.value)
const activeFilterChips = computed(() => {
  ...
  if (coverage.value) chips.push({ key: 'coverage', label: '覆盖状态', value: coverage.value === 'uncovered' ? '未覆盖' : '已覆盖', clear: () => { coverage.value = '' } })
...
const filteredEndpoints = computed(() => {
  ...
  return source
    ...
    .filter((item) => !coverage.value || (coverage.value === 'covered' ? item.covered : !item.covered))
...
function groupEndpoints(items: ApiEndpointItem[]) {
  ...
  return Array.from(groups.entries()).map(([key, group]) => {
    ...
    return {
      ...
      covered: group.every((item) => item.covered),
...
function toggleUncoveredOnly() {
  coverage.value = coverage.value === 'uncovered' ? '' : 'uncovered'
}
...
async function copyUncovered() {
  const text = filteredEndpoints.value.filter((item) => !item.covered).map((item) => item.endpointKey).join('\n')
  await copyText(text, `已复制 ${text ? text.split('\n').length : 0} 个未覆盖接口`)
}

function exportCsv() {
  const rows = [['接口类型', 'HTTP 方法', 'URL', '覆盖状态', '命中次数', '类名', '方法', '关联用例']]

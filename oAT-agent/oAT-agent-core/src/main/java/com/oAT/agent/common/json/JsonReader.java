...
public class JsonReader implements Closeable
{
    ...
    public static Map jsonToMaps(String json, Map<String, Object> optionalArgs)
    {
        ...
        {
            int initialCapacity = json == null ? 16 : Math.max(16, json.length() / 100);
            optionalArgs = new HashMap<String, Object>((int) (initialCapacity / 0.75f) + 1);
        }
        ...
        if (json != null) {
            try {
                ...
                throw new JsonIoException("UTF-8 not supported", e);
            }
        } else {
            // json为null时直接返回空Map
    ...
    public static Map jsonToMaps(InputStream inputStream, Map<String, Object> optionalArgs)
    {
        if (optionalArgs == null)
        {
            optionalArgs = new HashMap<String, Object>(2);
        }
        optionalArgs.put(USE_MAPS, true);
        JsonReader jr = new JsonReader(inputStream, optionalArgs);
        Object ret = jr.readObject();
    ...
    public Object readObject()
    {
        ...
        // USE_MAPS为true时直接返回解析结果，避免递归死循环
        if (useMaps()) {
            return o;
        }

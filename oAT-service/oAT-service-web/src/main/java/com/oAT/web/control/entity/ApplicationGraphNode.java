...
public class ApplicationGraphNode extends GraphNode {
    ...
    private List<SqlTraceGroup> groupBySql() {
        Function<SqlTraceNode, SqlTraceGroup> groupFunction = node -> new SqlTraceGroup(node.getDatabase().getAddressIp(),
                node.getDatabase().getPort(), node.getDatabase().getName(), node.getDatabase().getType(), node.getSql());

...
<script>
    ...
    function submitEditApp(){var $form=$('#editAppForm');if(oatIsFormSubmitting($form)){return}if(editor){editor.save()}if(!$form.form('is valid')){return}var data=$form.serialize();oatSetFormSubmitting($form,true,{submitButton:'#editAppSubmitButton',editor:editor,extraControls:'.project-settings-hero-actions .ui.button',message:'正在保存应用...'});$.post('/p/${project.id}/app/doEdit',data,function(res){if(res.success||res.result){showToast(res.message||'应用修改成功','success');setTimeout(function(){location.href='/p/${project.id}/app/list'},1000)}else{oatSetFormSubmitting($form,false,{submitButton:'#editAppSubmitButton',editor:editor});showToast(res.message||'应用修改失败','error')}}).fail(function(){oatSetFormSubmitting($form,false,{submitButton:'#editAppSubmitButton',editor:editor});showToast('网络请求失败','error')})}

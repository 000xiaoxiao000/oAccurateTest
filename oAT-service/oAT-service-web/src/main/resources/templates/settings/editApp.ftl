        ...
        <div class="project-settings-main">
            ...
            <div class="project-settings-form-wrap">
                <form id="editAppForm" class="ui form project-settings-form">
                    ...
                    <div class="project-settings-section"><div class="project-settings-section-title"><i class="bell outline icon"></i>探针上下线告警</div><p class="project-alert-desc">按每个探针实例独立判断上下线，超过配置阈值未收到心跳后发送通用 Webhook 通知。测试时可填当前服务内置地址：<code>http://127.0.0.1:8899/webhook/oat/probe-alert</code></p>
                    ...
                    <div class="project-settings-actions"><a class="ui button" href="/p/${project.id}/app/list">取消并返回</a><button class="ui button" type="reset">重置</button><button class="ui primary button" type="button" onclick="submitEditApp()"><i class="save outline icon"></i>保存应用</button></div>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    function submitEditApp(){if(editor){editor.save()}var $form=$('#editAppForm');if(!$form.form('is valid')){return}$.post('/p/${project.id}/app/doEdit',$form.serialize(),function(res){if(res.success||res.result){showToast(res.message||'应用修改成功','success');setTimeout(function(){location.href='/p/${project.id}/app/list'},1000)}else{showToast(res.message||'应用修改失败','error')}}).fail(function(){showToast('网络请求失败','error')})}

...
└── oAT-service-web/  # 后端主服务，负责 API、快照、覆盖率、版本、用例等能力
```

## 构建

构建服务端前，需要先构建根目录下的 `oAT-agent`，因为 `oAT-service-web` 依赖 `oAT-agent/oAT-client-model`：
...
| `oAT-service-web` | war | 平台 API、Agent 上报、快照、覆盖率报告、版本中心、用例中心和 AI 对话入口 |
